...
- **Signed in:** server-authoritative for game mechanics, but the journal *content* is end-to-end encrypted (see below). `saveEntry()` encrypts the entry client-side, then calls the `save_entry` RPC; streak/XP/badges/once-per-day are computed in Postgres and the client applies the returned truth (`applyServerSave()`). Boot hydrates from the server (`loadRemote()`), decrypting each row; `localStorage` is just an offline cache (of already-decrypted plaintext, on-device only). Entries saved while offline queue in `gc-pending` (already encrypted before queuing) and flush on reconnect (`flushPending()`).

**End-to-end encryption (added post-v0.5):** `reflection`/`gratitudes` are encrypted in the browser (AES-GCM 256, via `window.crypto.subtle` — no libraries) with a key derived from the user's own password (PBKDF2-HMAC-SHA256, 600k iterations, per-user random salt in `profiles.encryption_salt`). The server only ever stores ciphertext (`entries.enc_blob`/`enc_iv`) — nobody with database access, including the project owner, can read journal text. The derived `CryptoKey` is cached in IndexedDB (`gc-keys`) per user id so a returning session on the same browser doesn't need the password retyped (`getCachedKey`/`cacheKey`/`clearCachedKey`); a session with no cached key prompts to unlock (`unlockWithPassword()`). A plaintext `reflection_length` column (just an integer, not content) lets `compute_badges` still award "Mind Full" without ever reading entry text. **Tradeoff to know:** a password reset (`forgotPassword()`) on the *same* device re-encrypts everything under the new password (`reencrypt_entries` RPC) so nothing is lost; a reset on a *different* device permanently locks entries written before the reset — nobody, including the owner, can recover them, since the key only ever existed as a derivation of the old password. Locked entries render with a lock icon instead of crashing.

**Schema (all RLS-protected):**
- `profiles` — 1:1 with `auth.users` (auto-created by trigger `handle_new_user`): name, streak, xp, last_entry_date, badges_earned[], shares, invites, challenge_count, encryption_salt. Clients can only directly UPDATE the `name` and `encryption_salt` columns (column-level grant); all stats change only via RPCs.
- `entries` — day (1–30), entry_date, prompt, enc_blob (base64 ciphertext), enc_iv (base64), reflection_length (int, plaintext length only — no content). `UNIQUE(user_id, entry_date)` + `UNIQUE(user_id, day)`. Clients can only SELECT their own; INSERT/UPDATE/DELETE are revoked entirely (immutable journal, writes only through RPCs).
- RPCs (SECURITY DEFINER, scoped to `auth.uid()`, authenticated-only): `save_entry(p_local_date, p_prompt, p_enc_blob, p_enc_iv, p_reflection_length)`, `migrate_guest_data(p_name, p_entries)` (one-time guest import, refuses if the account already has entries; expects pre-encrypted entries from the client), and `reencrypt_entries(p_updates)` (bulk re-encrypt on same-device password reset). `compute_badges(uuid)` is internal (not callable by clients).
- Security advisors show three expected WARNs (the three RPCs above are intentionally client-callable SECURITY DEFINER functions — that's the design since direct table writes are revoked).

**Auth:** email + password only. **"Confirm email" is currently ON** (Supabase default) — real users get a confirmation email before they can sign in; the client shows "Check your email" after signup. Toggle it off in Dashboard → Authentication for instant beta signups (test accounts were confirmed via SQL). Guest→account migration: on first sign-in, if local entries exist and the account is empty, they're encrypted client-side and pushed via `migrate_guest_data`, and the old local state is kept at `gc-state-backup`.
